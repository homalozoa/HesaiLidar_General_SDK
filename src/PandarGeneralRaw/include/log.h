...
public:
    TranceFunc(const char* file, const char* func){
        ...
        m_cFunc = func;
        struct tm *ptm;
        struct timeval stTimeb;
        gettimeofday(&stTimeb, NULL);
        ptm = localtime(&stTimeb.tv_sec);
        printf("[T] %02d:%02d:%02d.%03d pid:%d tid:%10d ->[File:%s Function:%s ]\n", ptm->tm_hour, ptm->tm_min, ptm->tm_sec, stTimeb.tv_usec, getpid(), std::hash<std::thread::id>()(std::this_thread::get_id()), m_cFile, m_cFunc);
    }
    ~TranceFunc(){
        struct tm *ptm;
        struct timeval stTimeb;
...
#define LOG_D(format,...) {\
    ...
    gettimeofday(&stTimeb, NULL);\
    ptm = localtime(&stTimeb.tv_sec);\
    printf("[D] %02d:%02d:%02d.%03d pid:%d tid:%10d File:%s Function:%s Line:%d " format"\n", ptm->tm_hour, ptm->tm_min, ptm->tm_sec, stTimeb.tv_usec, getpid(), std::hash<std::thread::id>()(std::this_thread::get_id()), __FILE__, __FUNCTION__, __LINE__, ##__VA_ARGS__);\
}
